...
async function wiki(config) {
  ...
  function analyzeUpgrades(upgrades,raceData,raceUnits = [],raceUpgrades = []) {
  ...
  function analyzeUnits(units, raceData, raceUnits = [],raceUpgrades = []){
    ...
    if(units) {
      for (let unitID of units) {
        ...
        if (!unitID || raceUnits.includes(unitID)) {
          continue;
        }
        ...
        analyzeUnits(unit._morph, raceData, raceUnits, raceUpgrades)
        analyzeUnits(unit._production, raceData, raceUnits, raceUpgrades)
        analyzeUpgrades(unit._research, raceData, raceUnits, raceUpgrades)
...
// })
  ...
  for (let raceData of racesData) {
    ...
    let startUnits = raceData.StartingUnitArray.filter(su => su.Unit && su.Count !== 0 && su.Flags?.ResourceSetRally).map(su => su.Unit)
    ...
    outputRaceData.army = units.filter((a) => mod.cache.unit[a]._category === 'army')

    outputRaceData.structures = units.filter((a) => mod.cache.unit[a]._category === 'structure')
